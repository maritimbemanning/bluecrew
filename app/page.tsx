...
function PageContent() {
  ...
  return (
    <main style={sx.page}>
      ...
      {/* SIKKERHET & BEREDSKAP */}
      <section id="sikkerhet" style={sx.sectionAlt}>
        <div style={sx.wrapNarrow}>
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            <h2 style={sx.h2}>Sikkerhet &amp; beredskap</h2>
            <p style={sx.leadSmall}>
              Sjøfolkene vi leverer har oppdatert sertifikatportefølje, og vi
              koordinerer beredskapsturnus for rederier og oppdrettsanlegg.
            </p>
          </div>

          <div style={sx.safetyGrid}>
            {SAFETY_POINTS.map((point) => (
              <article key={point.title} style={sx.safetyCard}>
                <div style={sx.safetyIcon} aria-hidden="true">
                  {point.icon}
                </div>
                <h3 style={sx.safetyTitle}>{point.title}</h3>
                <p style={sx.safetyText}>{point.text}</p>
              </article>
            ))}
          </div>
          ...
          <div style={sx.statsRow}>
            {SAFETY_STATS.map((stat) => (
              <div key={stat.label} style={sx.statItem}>
...
const SAFETY_POINTS = [
  {
    ...
    text: "Operativ bemanningsvakt følger opp beredskapsskift og sørger for rask utskifting ved sykdom eller utvidelse.",
  },
  {
    icon: "📑",
    title: "Oppdatert sertifikatbank",
    text: "Vi kontrollsjekker STCW, sikkerhetskurs, brovakt og helseattester før oppstart – og følger opp fornyelser.",
  },
  {
    icon: "🤝",
    title: "Lokalt nettverk",
    text: "Mannskap fra hele Nord-Norge og kysten ellers gir kort responstid til både oppdrett, fiskeri og servicefartøy.",
  },
];

const SAFETY_STATS = [
  { value: "98%", label: "Oppdrag levert til avtalt tidspunkt" },
  { value: "12 t", label: "Gjennomsnittlig responstid ved hasteoppdrag" },
  { value: "250+", label: "Kvalifiserte sjøfolk i aktivt nettverk" },
];
...
const sx: Record<string, React.CSSProperties> = {
  ...
  statLabel: { fontSize: 14, opacity: 0.85, lineHeight: 1.5 },
